...
export default function parseUserWatchHistory (html: string): ParserResult {
  ...
  const schema: ParserFieldsSchemaPaylist = {
    ...
    viewCount ({ $ }: ParserFieldParams): number {
      const viewCount = $('div#stats > .style-scope.ytd-playlist-sidebar-primary-info-renderer:nth-child(2)').text()
    ...
    videoCount ({ $ }: ParserFieldParams): number {
      const videoCount = $('div#stats > .style-scope.ytd-playlist-sidebar-primary-info-renderer:nth-child(1)').text()
    ...
    updatedAtString ({ $ }: ParserFieldParams): string {
      const updatedAt = $('div#stats > .style-scope.ytd-playlist-sidebar-primary-info-renderer:nth-child(3)').text()
      if (!updatedAt) throw new HarkeParsingError()

      return updatedAt
    },

    videos ({ $ }: ParserFieldParams): (PlaylistVideo|PlaylistVideoUnavailable)[] {
      const result: (PlaylistVideo|PlaylistVideoUnavailable)[] = []

      $('#contents > .ytd-playlist-video-list-renderer').each((idx, el: cheerio.Element) => {
        const $el = $(el)
